...
public class SessionResultsFormatterTest {
	...
	public void verboseOutputIncludesDetailedReasonAndPrettyPrintedCodeLocation() throws Exception {
		Collection<CheckerReasonDetail> reasons = Arrays.asList(
				new CheckerReasonDetail("1st checker reason message", 
						fromSlashed(slashed("path/to/MyClass")), 
						NOT_DECLARED_FINAL),
				new CheckerReasonDetail("2nd checker reason message", 
						FieldLocation.fieldLocation("myField", fromInternalName("path/to/OtherClass")), 
		...
		IAnalysisSession analysisSession = mock(IAnalysisSession.class);
		Collection<AnalysisResult> analysisResults = Arrays.asList(
				new AnalysisResult("a.b.c", IsImmutable.NOT_IMMUTABLE, reasons));
		when(analysisSession.getResults()).thenReturn(analysisResults);
